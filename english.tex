...
\citem{Intel}\\
\begin{itemize}
\item \textbf{Senior Software Engineer} (February 2015 -- present)\\
  ...
  YP projects (more than 100 commits). Main maintainer of 'patchtest', the framework and
  test suite testing Openembedded-Core community patches. Technical
  lead at GDC (Intel M\'exico, Guadalajara Design Center).
\end{itemize}

\citem{Freescale}\\
July 2012 -- February 2015
\begin{itemize}
\item \textbf{Professional Services Consulter} (April 2014 -- February 2015)\\
  Board Support Package (BSP) development and support for \textit{i.MX} custom
  boards. Design and develop user-space (multimedia) applications for the \textit{i.MX}
  processors.
\end{itemize}
...
\item \textbf{Field Application Engineer} (July 2012 -- April 2014)\\
  ...
  \textit{i.MX} Applications Processors, with main focus on issues raised from software
  coming from the \textit{meta-fsl-arm} layer. During this period, the company
  moved from a in-house Linux Distribution builder (\textit{ltib}) to the Yocto Project
...
\item \textbf{Embedded Software Engineer} (August 2007 -- June 2011).\\
  Creating and maintenance of GStreamer plugins for the \textit{OMAP} family
  processors. Plugins based on TI \textit{OpenMAX} IL multimedia interface.
  Maintainer of the camera and video encoder plugins.
\item \textbf{Embedded Software Engineer} (August 2006 -- August 2007)\\
  Software developer for the \textit{OpenMAX} IL Camera component for
  \textit{OMAP2} family processors.
\end{itemize}
\end{category}
...
\citembullet Currently taking Organ classes
